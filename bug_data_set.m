...
% for i = 1: size(trainImages,1)
   try
    nameof = '10405299_1';
    file_num        = regexpi(nameof,'\d*(?=\_)','match');
    subject_num     = regexpi(nameof,'(?<=_)\d*','match'); 
    imagePath       = strcat(helenTrainFolder,'\',nameof,'.jpg');
...
%     saveas(h,char(strcat('results/',file_num,'_',subject_num, '.jpg')))
    ...
    allGroundTruths(1,1) = {helenGroundTruth};
    allGroundTruths(1,2) = {chlandmarkPoints};
    allGroundTruths(1,3) = {final_landmarks};
